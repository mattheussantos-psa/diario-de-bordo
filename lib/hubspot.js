...
// A propriedade existe nesta conta? Evita mandar um campo que o HubSpot não
// conhece, o que faz ele recusar o PATCH inteiro — e levaria junto a
// temperatura, que é a parte que não pode falhar.
export async function temPropriedadeDeal(nome) {
  return cached(`prop-existe-${nome}`, UMA_HORA, async () => {
    try {
      await hs(`/crm/v3/properties/deals/${nome}`);
      return true;
    } catch {
      return false;
    }
...
export async function updateDeal(id, { temperatura_atual, observacoes, estrategia_do_dia }) {
  const properties = {};
  ...
  if (estrategia_do_dia !== undefined) properties.estrategia_do_dia = estrategia_do_dia;
