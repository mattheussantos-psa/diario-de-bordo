...
// Associações de um tipo de engajamento com empresas, em lote.
export async function associacoesPara(objeto, ids) {
  const out = {};
  const lista = [...new Set((ids || []).map(String))];
  for (let i = 0; i < lista.length; i += 100) {
    const chunk = lista.slice(i, i + 100);
    const r = await hs(`/crm/v4/associations/${objeto}/companies/batch/read`, {
      method: "POST",
      body: JSON.stringify({ inputs: chunk.map((id) => ({ id })) }),
    });
    for (const row of r.results || []) {
