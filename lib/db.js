...
export async function getListaFunil(ownerId, dia) {
  ...
  try {
    await init();
    const sql = db();
    const rows = await sql`SELECT deal_id, grupo FROM funil_dia
      WHERE owner_id=${String(ownerId)} AND dia=${dia} ORDER BY posicao`;
    return rows.map((r) => ({ id: r.deal_id, grupo: r.grupo }));
  } catch (e) {
...
export async function salvarListaFunil(ownerId, dia, negocios) {
  ...
  await sql`INSERT INTO funil_dia (owner_id, dia, deal_id, grupo, posicao)
    SELECT ${String(ownerId)}, ${dia}, d, g, p
    FROM unnest(${ids}::text[], ${grupos}::text[], ${pos}::int[]) AS t(d, g, p)
