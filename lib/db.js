...
import { buildItemArrays } from "./plan-insert";
...
// Histórico: planos de semanas anteriores, com a contagem de negócios.
export async function getPlansHistory(currentWeek, limit = 200) {
  if (!dbReady()) return [];
  try {
    await init();
    const sql = db();
    const rows = await sql`
      SELECT p.owner_id, p.week, p.status, p.motivo, p.revisado_por, p.updated_at,
        (SELECT COUNT(*) FROM plan_items i WHERE i.owner_id = p.owner_id AND i.week = p.week) AS n
      FROM plans p
      WHERE p.week <> ${currentWeek}
      ORDER BY p.week DESC, p.owner_id
      LIMIT ${limit}`;
    return rows.map((r) => ({
      ownerId: r.owner_id,
      week: r.week,
    ...
    console.error("[plano] falha no histórico:", e?.message);
    return [];
  }
}
...
export async function savePlan(ownerId, week, items, status) {
  await init();
  const sql = db();
  // status null = preserva o status atual (usado quando o gestor só ajusta os itens).
  await sql`INSERT INTO plans (owner_id, week, status, updated_at)
    VALUES (${String(ownerId)}, ${week}, COALESCE(${status}, 'rascunho'), now())
    ON CONFLICT (owner_id, week) DO UPDATE
    SET status = COALESCE(${status}, plans.status), updated_at = now()`;
  ...
  const { dealIds, dias } = buildItemArrays(items);
  if (dealIds.length) {
    await sql`INSERT INTO plan_items (owner_id, week, deal_id, dia)
      SELECT ${String(ownerId)}, ${week}, d, x
      FROM unnest(${dealIds}::text[], ${dias}::smallint[]) AS t(d, x)`;
  }
}
