...
export async function getBriefing(ownerId, dia) {
  ...
  try {
    ...
    const [cab, itens] = await Promise.all([
      ...
      sql`SELECT deal_id, de, para FROM briefing_items WHERE owner_id=${String(ownerId)} AND dia=${dia}`,
    ]);
    const row = cab[0];
    const items = {};
    for (const it of itens) items[it.deal_id] = { de: it.de || "", para: it.para || "" };
    return {
...
export async function getDayBriefings(dia) {
  ...
  try {
    ...
    const [cab, itens] = await Promise.all([
      ...
      sql`SELECT owner_id, deal_id, de, para FROM briefing_items WHERE dia=${dia}`,
    ]);
    const porOwner = {};
    for (const c of cab) {
      porOwner[c.owner_id] = {
    ...
    for (const it of itens) {
      if (porOwner[it.owner_id]) {
        porOwner[it.owner_id].items[it.deal_id] = { de: it.de || "", para: it.para || "" };
      }
    }
...
export async function getBriefingsForDays(dias) {
  ...
  try {
    ...
    const [cab, itens] = await Promise.all([
      ...
      sql`SELECT owner_id, dia, deal_id, de, para FROM briefing_items WHERE dia = ANY(${dias}::date[])`,
    ]);
    ...
    for (const it of itens) {
      ...
      mapa[k] ||= { ownerId: it.owner_id, dia: toKey(it.dia), status: "rascunho", items: {} };
      mapa[k].items[it.deal_id] = { de: it.de || "", para: it.para || "" };
    }
...
// Histórico: dias anteriores, com a contagem de negócios.
export async function getBriefingHistory(diaAtual, limit = 200) {
  if (!dbReady()) return [];
  try {
    await init();
    const sql = db();
...
export async function saveBriefing(ownerId, dia, items, status) {
  ...
  await sql`INSERT INTO briefings (owner_id, dia, status, updated_at)
    VALUES (${String(ownerId)}, ${dia}, COALESCE(${status}, 'rascunho'), now())
    ON CONFLICT (owner_id, dia) DO UPDATE
    SET status = COALESCE(${status}, briefings.status), updated_at = now()`;
  ...
  if (entries.length) {
    ...
    await sql`INSERT INTO briefing_items (owner_id, dia, deal_id, de, para)
      SELECT ${String(ownerId)}, ${dia}, d, x, y
      FROM unnest(${ids}::text[], ${des}::text[], ${paras}::text[]) AS t(d, x, y)`;
  }
}
