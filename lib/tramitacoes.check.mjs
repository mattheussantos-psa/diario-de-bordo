...
// Onboarding agendado para o futuro é agendamento, não realização: o prazo
// só começa quando a data chega.
const futuro = { id: "9", onboarding: "2026-12-01", statusContrato: "Pendente" };
ok(pendenciasDoTicket(futuro, "2026-09-23").length === 0, "onboarding futuro não cobra nada hoje");
ok(acha(pendenciasDoTicket(futuro, "2026-12-01"), "minuta") !== undefined, "no dia do onboarding a minuta entra");

// Evento já realizado: o ticket continua aberto, mas a ação perdeu a hora.
const passado = { id: "10", evento: "2026-09-01", statusContrato: "Pendente" };
ok(acha(pendenciasDoTicket(passado, "2026-09-23"), "checklist")?.eventoPassado === true, "evento passado é sinalizado");
const futuroEvento = { id: "11", evento: "2026-09-24", statusContrato: "Pendente" };
ok(acha(pendenciasDoTicket(futuroEvento, "2026-09-23"), "checklist")?.eventoPassado === false, "evento por vir não é passado");
